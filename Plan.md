...
- 最近任务同步：
  ...
  - 2026-04-18：局部修复完成（DMX Animation Layer / Ellis 复杂样例）。已在 [ImportPolicy.h](dcc_plugin/src/common/ImportPolicy.h) 新增“仅复用现有动画目标”判定，并将 [DmxImportDag.cpp](dcc_plugin/src/importer/DmxImportDag.cpp) 的缺失 DAG/shape 处理收紧到 `UsesUpdateCurrentScene + UsesAnimationLayerImport` 组合路径：当 `mechanic_model_merged.dmx` 作为基础场景，再以 `useSceneRoot=1;importMode=update;importAnimationToLayer=1;animationLayerMode=replace` 导入 `animation/c5m1_intro_mechanic.dmx` 时，不再补建缺失 helper DAG，局部 `mayapy` 回归结果已从“新增 61 个 transform”收敛为“新增 0 个 transform”，同时 `c5m1_intro_mechanic_dmx_layer` 仍成功生成 468 条 animation layer 曲线。当前结论：这条修复已覆盖本次 `Update + Animation Layer` 误补建节点问题，后续若要进一步固化，可再把该 Ellis 组合补进自动化 gate。
